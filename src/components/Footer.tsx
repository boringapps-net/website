  ...
  return (
    <footer className="bg-[#1A1A1A] border-t-4 border-[#1A1A1A] text-gray-300 py-12 px-6 md:px-12 mt-auto font-sans">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
        {/* Col 1: Brand & Joint Venture Pitch */}
        <div className="md:col-span-2 space-y-4">
          ...
          <p className="text-xs text-gray-400 leading-relaxed max-w-sm">
            We build unsexy (actually, beautiful, secure and blazing-fast) native utilities that run completely on your local CPU. No unnecessary telemetry, no cloud subscriptions, no memory leaks. Just robust software.
          </p>
          ...
          <div className="pt-2 text-[11px] text-gray-500 font-mono space-y-1 block border-t border-gray-800">
            ...
            <span className="block">• {CORPORATE_INFO.partners[0].name} (ABN {CORPORATE_INFO.partners[0].abn})</span>
            <span className="block">• {CORPORATE_INFO.partners[1].name} (ABN {CORPORATE_INFO.partners[1].abn})</span>
          </div>
        </div>

        {/* Col 2: App Suite Links */}
        <div>
          <h4 className="text-xs uppercase font-mono tracking-wider text-white font-extrabold mb-4 border-b border-gray-800 pb-1">Our Utility Suite</h4>
          <ul className="space-y-2.5 text-xs font-mono">
            {APPS.map((app) => (
              <li key={app.id}>
                ...
                >
                  {app.name} <span className="text-gray-500 text-[10px]">— {app.platforms.join(' / ')}</span>
                </button>
              </li>
            ))}
          </ul>
        ...
        {/* Col 3: Legal Disclosures & Documents */}
        <div>
          <h4 className="text-xs uppercase font-mono tracking-wider text-white font-extrabold mb-4 border-b border-gray-800 pb-1 font-sans">Regulatory Documents</h4>
          <ul className="space-y-2.5 text-xs font-mono">
            <li>
              <button
      ...
      <div className="max-w-7xl mx-auto border-t border-gray-800 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-[11px] text-gray-500 font-mono">
        <p>
          &copy; {currentYear} {CORPORATE_INFO.jointVentureName}. All rights reserved strictly throughout Australia.
        </p>
